...
func _ready():
	...
	# Inicializuojam pinigus Global.money, jei dar nebuvo nustatyta
	Global.money = 50
...
func _physics_process(delta):
	# Start day when doors open again
	...
	if day_end_pending:
		if watched_door.global_position.y > door_start_y:
			watched_door.global_position.y = max(
				watched_door.global_position.y - 0.05,
				door_start_y
			)
		else:
			_finalize_day_end()
...
func _finalize_day_end():
	...
	timeLabel_2d.text = "%s %02d:%02d" % [
		["Pr","An","Tr","Kt","Pn","Št","Sk"][current_day % 7],
		day_start_hour,
		0
	]
...
func _on_food_button_pressed(index: int):
	...
	if !order_confirmed_correct:
		# Tik prieš Confirm pildom selected_items
